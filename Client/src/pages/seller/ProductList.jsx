  ...
  const handleToggleStock = async (id, inStock) => {
    try {
      ...
      if (data.success) {
        ...
        // Refresh page or trigger product fetch here if needed
      } else {
            ...
            <tbody className="text-sm text-gray-500">
              {products.map((product) => (
                <tr key={product._id} className="border-t border-gray-500/20">
                  ...
                  <td className="px-4 py-3 max-sm:hidden">${product.offerPrice.toFixed(2)}</td>
                  <td className="px-4 py-3">
                    <label className="relative inline-flex itedms-center cursor-pointer text-gray-900 gap-3">
                      <input
                        type="checkbox"
